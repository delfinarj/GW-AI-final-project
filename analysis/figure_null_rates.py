...
def main():
    ...
    ax.text(ALPHA, len(names) - 0.4, f" α = {ALPHA}", color=ps.INK_SECONDARY, fontsize=8, va="bottom")
    ax.set_yticks(range(len(names)), [LABELS[n] for n in names])
    ax.invert_yaxis()
    ax.set_ylim(len(names) - 0.3, -0.7)
    x_max = max(max(data[n]["ci95"][1] for n in names) * 1.25, ALPHA * 3)
    # a little room left of zero so markers at a rate of 0 are drawn whole, not cut by the axis
